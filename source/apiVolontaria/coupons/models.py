...
class RechargeableCoupon(models.Model):
    ...
    def clean(self):
        ...
        if RechargeableCoupon.objects.exclude(id=self.id).filter(code=self.code):
...
class CouponOperation(models.Model):
    coupon = models.ForeignKey(RechargeableCoupon, null=True, on_delete=models.CASCADE)
...
class Coupon(models.Model):
    ...
    def amount(self):
        if self.presence_duration_minutes:
            return round(self.presence_duration_minutes/60 * 24)

    @staticmethod
    def generate_unique_code(cycle, user):
        try:
            code_start = user.email.split('@')[0]
        except:
            code_start = "nousrire"
    ...
    def clean(self):
        if not self.code:
            self.code = self.generate_unique_code(self.cycle, self.user)
